...
def create_agent_run_record(
    *,
    ...
    try:
        ...
        connection.close()
    add_agent_run_event(run_id, "queued", "运行已登记，等待 Agent 执行。")
...
def get_agent_run(run_id: str) -> dict[str, Any] | None:
    ...
    try:
        row = connection.execute("SELECT * FROM agent_runs WHERE id = ?", (run_id,)).fetchone()
        return agent_run_row(row) if row else None
    finally:
        connection.close()
...
def list_agent_runs(project_id: str, *, session_id: str = "", limit: int = 20) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [agent_run_row(row) for row in rows]
    finally:
        connection.close()
...
def agent_run_timeline(run_id: str) -> dict[str, Any] | None:
    ...
    return {
        "run": run,
        "events": list_agent_run_events(run_id, limit=200),
        "actions": [agent_action_row(row) for row in action_rows],
...
def agent_run_summary(project_id: str, batch: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "active": sum(counts.get(status, 0) for status in ("queued", "running")),
            "failed": counts.get("failed", 0),
            "latest": agent_run_row(latest_row) if latest_row else None,
        }
    finally:
        connection.close()
...
def get_idea_session(session_id: str) -> dict[str, Any] | None:
    ...
    try:
        row = connection.execute("SELECT * FROM idea_sessions WHERE id = ?", (session_id,)).fetchone()
        return idea_session_row(row) if row else None
    finally:
        connection.close()
...
def list_idea_sessions(limit: int = 20) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [idea_session_row(row) for row in rows]
    finally:
        connection.close()
...
def list_idea_hypotheses(session_id: str, plan_version: int | None = None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [idea_hypothesis_row(row) for row in rows]
    finally:
        connection.close()
...
def list_idea_validation_tasks(session_id: str, plan_version: int | None = None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [idea_validation_task_row(row) for row in rows]
    finally:
        connection.close()
...
def list_idea_decisions(session_id: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [idea_decision_row(row) for row in rows]
    finally:
        connection.close()


def idea_validation_plan(session_id: str) -> dict[str, Any]:
    decisions = list_idea_decisions(session_id)
    latest_version = int(decisions[0]["version"]) if decisions else 0
    all_hypotheses = list_idea_hypotheses(session_id)
    all_tasks = list_idea_validation_tasks(session_id)
    latest_hypotheses = list_idea_hypotheses(session_id, latest_version) if latest_version else all_hypotheses
    latest_tasks = list_idea_validation_tasks(session_id, latest_version) if latest_version else all_tasks
...
def create_idea_hypothesis_record(
    *, session_id: str, plan_version: int = 0, statement: str, category: str = "需求", priority: str = "normal",
    ...
    try:
        ...
        return idea_hypothesis_row(row)
    finally:
        connection.close()
...
def create_idea_validation_task_record(
    *, session_id: str, hypothesis_id: int, plan_version: int = 0, title: str, task_type: str = "interview", due_at: str = "",
    ...
    try:
        ...
        return idea_validation_task_row(row)
    finally:
        connection.close()
...
def attach_idea_task_work_item(task_id: int, work_item_id: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        return idea_validation_task_row(row) if row else None
    finally:
        connection.close()
...
def get_agent_session(session_id: str, project_id: str = "") -> dict[str, Any] | None:
    ...
    try:
        if project_id:
            ...
            row = connection.execute("SELECT * FROM agent_sessions WHERE id = ?", (session_id,)).fetchone()
        return agent_session_row(row) if row else None
    finally:
        connection.close()
...
def list_agent_sessions(project_id: str, limit: int = 20) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [agent_session_row(row) for row in rows]
    finally:
        connection.close()
...
def list_agent_messages(session_id: str, limit: int = 40) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [agent_message_row(row) for row in reversed(rows)]
    finally:
        connection.close()
...
def get_agent_action_record(action_id: str) -> dict[str, Any] | None:
    ...
    try:
        row = connection.execute("SELECT * FROM agent_actions WHERE id = ?", (action_id,)).fetchone()
        return agent_action_row(row) if row else None
    finally:
        connection.close()
    ...
    try:
        for path in paths:
            try:
                ...
                continue
            text = _embedding_text_for_note(path, content)
    ...
    try:
        ...
        for row in rows:
            ...
            if len(stored) != len(query_vector):
                continue
            scores[str(row["path"])] = _cosine_similarity(query_vector, stored)
    finally:
        connection.close()
    return scores
...
def knowledge_hybrid_search(query: str = "", limit: int = 50) -> list[dict[str, Any]]:
    ...
    keyword = knowledge_search(query)
    if not query.strip() or not embedding_available():
        return keyword[:limit]
    ensure_knowledge_vectors()
    semantic = knowledge_semantic_scores(query)
    if not semantic:
        return keyword[:limit]
    max_sim = max(semantic.values()) or 1.0
    merged: dict[str, dict[str, Any]] = {}
    for note in keyword:
        absolute = str(KNOWLEDGE_DIR / note["path"])
...
def obsidian_note_paths() -> list[Path]:
    ...
    for path in OBSIDIAN_VAULT_DIR.rglob("*.md"):
        ...
        try:
            ...
            resolved.relative_to(OBSIDIAN_VAULT_DIR.resolve())
...
def obsidian_note_row(row: sqlite3.Row) -> dict[str, Any]:
    ...
    note["vault_path"] = str(OBSIDIAN_VAULT_DIR)
    return note
...
def obsidian_index_vault() -> dict[str, Any]:
    started = now_iso()
    paths = obsidian_note_paths()
    if not OBSIDIAN_VAULT_DIR.is_dir():
        return {"ok": False, "vault_path": str(OBSIDIAN_VAULT_DIR), "error": "Obsidian Vault 不存在或不可读取", "indexed": 0}
    indexed_paths: set[str] = set()
    changed = 0
    connection = db_connection()
    try:
        for path in paths:
            relative = str(path.relative_to(OBSIDIAN_VAULT_DIR))
            ...
            try:
                ...
                continue
            parsed = parse_obsidian_markdown(text, path)
        ...
        connection.close()
    return {"ok": True, "vault_path": str(OBSIDIAN_VAULT_DIR), **summary}
...
def obsidian_status() -> dict[str, Any]:
    ...
    try:
        ...
        return {"available": OBSIDIAN_VAULT_DIR.is_dir(), "vault_path": str(OBSIDIAN_VAULT_DIR), "note_count": count, "today_note_count": today_count, "moc_count": moc_count, "last_indexed_at": latest, "last_scan": last_scan}
    finally:
        connection.close()
...
def obsidian_search(query: str = "", limit: int = 40, since_timestamp: float = 0) -> list[dict[str, Any]]:
    rows = [row for row in obsidian_index_rows() if not since_timestamp or float(row["mtime"] or 0) >= since_timestamp]
    ...
    for score, _, row in ranked[: max(1, min(limit, 100))]:
        note = obsidian_note_row(row)
        note["score"] = score
        note["backlinks"] = obsidian_backlink_count(note["path"], note["title"], all_rows)
        results.append(note)
    return results


def obsidian_related(path: str, limit: int = 8) -> list[dict[str, Any]]:
    rows = obsidian_index_rows()
    target_row = next((row for row in rows if str(row["path"]) == str(path)), None)
    if not target_row:
        return []
    target = obsidian_note_row(target_row)
    ...
    for row in rows:
        ...
        note["backlinks"] = obsidian_backlink_count(note["path"], note["title"], rows)
        related.append(note)
...
def obsidian_moc_suggestions() -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if not str(row["folder"] or "").startswith("03 MOC") and obsidian_backlink_count(note["path"], note["title"], rows) == 0:
...
def knowledge_inbox_candidates() -> list[dict[str, Any]]:
    ...
    for item in list_inbox("inbox"):
        ...
        existing = knowledge_search(title)[:2] if len(title) >= 2 else []
        candidates.append({
            "id": item["id"],
...
def sync_inbox_to_obsidian(item_id: int, *, content: str = "", title_override: str = "") -> dict[str, Any]:
    ...
    for artifact in list_artifacts("knowledge"):
        metadata = artifact.get("metadata") if isinstance(artifact.get("metadata"), dict) else {}
    ...
    inbox_dir = OBSIDIAN_VAULT_DIR / "00 Inbox"
    ...
    title = clip(title, 70)
    filename = safe_filename(f"{title}-{item_id}", f"workbench-inbox-{item_id}") + ".md"
    path = inbox_dir / filename
    suffix = 2
    while path.exists():
        path = inbox_dir / f"{safe_filename(title, f'workbench-inbox-{item_id}')}-{item_id}-{suffix}.md"
        suffix += 1
    ...
    artifact = register_artifact_safely(
        ...
        metadata={"title": title, "source_inbox_id": item_id, "confirmed_at": now_iso(), "vault_relative_path": str(path.relative_to(OBSIDIAN_VAULT_DIR))},
    )
    ...
    try:
        create_notification_record(
            ...
            body=f"收件箱 #{item_id} 已写入 {path.relative_to(OBSIDIAN_VAULT_DIR)}，原收件箱内容保留。",
            project_id="knowledge",
            kind="knowledge_sync",
            level="info",
...
def knowledge_draft_source_check(draft: dict[str, Any]) -> dict[str, Any]:
    ...
    for source_id in source_ids[:30]:
        source = get_artifact_record(source_id)
        if not source:
            ...
            continue
        source_text, source_error = read_artifact_source(source)
    ...
    for source_note in [item for item in metadata.get("source_notes", []) if isinstance(item, dict)][:30]:
        ...
        if not relative:
            continue
        note_path = (OBSIDIAN_VAULT_DIR / relative).resolve()
        try:
            note_path.relative_to(OBSIDIAN_VAULT_DIR.resolve())
        except ValueError:
...
def sync_knowledge_draft_to_obsidian(artifact_id: int, *, conflict_action: str = "new") -> dict[str, Any]:
    ...
    source_check = knowledge_draft_source_check(artifact)
    ...
    base = safe_filename(title, f"workbench-draft-{artifact_id}")
    ...
    destination = register_artifact_safely(
        project_id="knowledge",
        name=path.name,
        path=str(path),
        kind="obsidian_reviewed_draft",
        metadata={"source_artifact_id": artifact_id, "confirmed_at": confirmed_at, "conflict_action": conflict_action, "backup": backup, "vault_relative_path": str(path.relative_to(OBSIDIAN_VAULT_DIR)), "source_check": source_check},
    )
    relation = create_relation_record(
    ...
    update_artifact_metadata(artifact_id, {"obsidian_sync": {"artifact_id": destination.get("id") if destination else None, "path": str(path), "confirmed_at": confirmed_at, "backup": backup, "source_check": source_check}})
    return {"synced": True, "skipped": False, "path": str(path), "vault_relative_path": str(path.relative_to(OBSIDIAN_VAULT_DIR)), "backup": backup, "artifact": destination, "relation": relation, "source_check": source_check, "confirmed_at": confirmed_at, "message": "草稿已写入 Obsidian Inbox，来源 Artifact、校验结果和备份已保留。"}


def knowledge_draft_replay(artifact_id: int) -> dict[str, Any]:
    """Return bounded source excerpts for a draft's citation replay UI."""
    draft = get_artifact_record(artifact_id)
    ...
    for source_id in source_ids[:20]:
        ...
        if not source:
            ...
            continue
        content, error = read_artifact_source(source)
        if error:
    ...
    return {
        ...
        "sources": sources,
        "source_check": knowledge_draft_source_check(draft),
...
def agent_project_context(project_id: str) -> dict[str, Any]:
    ...
    if project_id == "inbox":
        ...
        context = {
            ...
            "recent_notes": knowledge_search("")[:12],
            "obsidian": obsidian_status(),
            "obsidian_recent_notes": obsidian_search("", limit=12),
            "obsidian_today_notes": obsidian_search("", limit=20, since_timestamp=datetime.now().astimezone().replace(hour=0, minute=0, second=0, microsecond=0).timestamp()),
            "inbox_candidates": knowledge_inbox_candidates()[:12],
            "moc_suggestions": obsidian_moc_suggestions(),
        }
        ...
        context = {
            ...
            "available_source_artifacts": document_factory_source_descriptors(limit=30),
        ...
        for durable in durable_runs:
            ...
            if not run_id or run_id in seen_run_ids:
                continue
            runtime = runs.get(run_id) or runtime_crawl_from_agent_run(durable)
            result = durable.get("result") or {}
        ...
        context = {"source": "aihot.today", "fetched_at": snapshot.get("fetched_at"), "items": select_aihot_items(snapshot, "useful", limit=12)}
    ...
    return {
        ...
        "shared_context": {
            ...
            "open_work_items": list_work_items("open", project_id)[:12],
            "recent_artifacts": list_artifacts(project_id)[:12],
        },
    }
...
def write_knowledge_note(title: str, content: str, *, metadata: dict[str, Any] | None = None, artifact_kind: str = "knowledge_note") -> dict[str, Any]:
    filename = safe_filename(title, "未命名笔记") + ".md"
    path = KNOWLEDGE_DIR / filename
    if path.exists():
        filename = f"{safe_filename(title, '未命名笔记')}-{datetime.now().strftime('%H%M%S')}.md"
        path = KNOWLEDGE_DIR / filename
    body = content.strip()
    ...
    return {"name": path.stem, "path": str(path.relative_to(KNOWLEDGE_DIR)), "title": title, "artifact": artifact}
...
def execute_agent_action(action_id: str, *, force: bool = False, parent_run_id: str = "") -> dict[str, Any]:
    ...
    if not run:
        ...
        add_agent_run_event(run["id"], "retry_queued", f"第 {next_attempt} 次尝试已排队。", metadata={"action_id": action_id})
    ...
    try:
        ...
        add_agent_run_event(run["id"], "succeeded", "动作执行完成。", level="success", metadata={"action_id": action_id})
        ...
        add_agent_run_event(run["id"], "failed", f"动作执行失败：{error}", level="error", metadata={"action_id": action_id})
        parent = get_agent_run(parent_run_id) if parent_run_id else None
        if not parent or parent.get("project_id") != "workbench":
            try:
                create_notification_record(
...
async def run_project_agent(
    *,
    ...
    try:
        ...
        add_agent_run_event(run["id"], "failed", f"{agent_display_name(project_id)} 调用失败：{error}", level="error")
        raise HTTPException(502, f"{agent_display_name(project_id)} 调用失败：{error}") from exc
    except HTTPException as exc:
        error = clip(str(exc.detail or exc), 800)
        update_agent_run_record(run["id"], status="failed", error=error)
        add_agent_run_event(run["id"], "failed", f"{agent_display_name(project_id)} 请求被能力边界拦截：{error}", level="error")
        raise
    except Exception as exc:
        error = str(exc)
        update_agent_run_record(run["id"], status="failed", error=error)
        add_agent_run_event(run["id"], "failed", f"{agent_display_name(project_id)} 执行失败：{error}", level="error")
        raise HTTPException(502, f"{agent_display_name(project_id)} 调用失败：{error}") from exc
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            "message": assistant_message,
            "result_contract": result_contract,
            "messages": list_idea_messages(session["id"], limit=100),
            "agent": agent_detail("idea-analysis", llm_ready=True),
        }
    except httpx.HTTPStatusError as exc:
        error = f"上游返回 {exc.response.status_code}：{clip(exc.response.text, 500)}"
        update_agent_run_record(run["id"], status="failed", error=error)
...
async def generate_idea_validation_plan(
    ...
    *,
    ...
    messages = list_idea_messages(session_id, limit=16)
    if not messages:
    ...
    try:
        ...
        plan = parse_idea_validation_plan(answer, session)
        decision = create_idea_decision_record(
            session_id=session_id,
        ...
        for item in plan["hypotheses"]:
            hypothesis = create_idea_hypothesis_record(
                session_id=session_id,
            ...
            hypothesis_relation = create_relation_record(
        ...
        for task_data in plan["tasks"]:
            ...
            task = create_idea_validation_task_record(
                session_id=session_id,
            ...
            task = attach_idea_task_work_item(task["id"], work_item["id"]) or task
            validation_relation = create_relation_record(
        ...
        session = update_idea_session_summary(session_id, summary) or session
        ...
        plan_result = {"session": session, "plan": idea_validation_plan(session_id), "decision": decision, "tasks": tasks, "answer": plan["raw_answer"], "result_contract": plan_contract}
...
async def get_knowledge(q: str = "", vector: int = 0) -> dict[str, Any]:
    if int(vector or 0) and embedding_available():
        notes = await asyncio.to_thread(knowledge_hybrid_search, q)
        return {"root": str(KNOWLEDGE_DIR), "notes": notes, "mode": "hybrid-vector"}
...
def knowledge_evaluation() -> dict[str, Any]:
    ...
    embedding_on = embedding_available()
    for item in queries:
        q = item["query"]
        keyword_hits = knowledge_search(q)[:8]
        keyword_titles = {note["path"] for note in keyword_hits}
        hybrid_hits = knowledge_hybrid_search(q)[:8] if embedding_on else []
...
async def reindex_knowledge_vectors(force: int = 0) -> dict[str, Any]:
    ...
    return await asyncio.to_thread(ensure_knowledge_vectors, bool(int(force or 0)))
...
def batch_sync_obsidian_inbox(request: ObsidianInboxBatchSyncRequest) -> dict[str, Any]:
    ...
    for item_id in item_ids:
        try:
            results.append({"id": item_id, "ok": True, "result": sync_inbox_to_obsidian(item_id)})
        except HTTPException as exc:
...
async def run_idea_analysis_opportunity(item_id: int) -> dict[str, Any]:
    ...
    session = create_idea_session(f"验证：{clip(str(signal.get('title') or item.get('title') or '未命名机会'), 90)}")
    ...
    try:
        ...
        updated_item = update_work_item_record(
            item_id,
            {
                "status": "done",
...
def get_idea_analysis_session(session_id: str) -> dict[str, Any]:
    ...
    if not session:
        raise HTTPException(404, "想法会话不存在")
    return {"session": session, "messages": list_idea_messages(session_id, limit=100)}
...
def get_idea_analysis_plan(session_id: str) -> dict[str, Any]:
    ...
    if not session:
        raise HTTPException(404, "想法会话不存在")
    return {"session": session, "plan": idea_validation_plan(session_id)}


@app.post("/api/idea-analysis/sessions/{session_id}/plan")
async def create_idea_analysis_plan(session_id: str) -> dict[str, Any]:
    return await generate_idea_validation_plan(session_id, trigger="manual")
...
async def chat_idea_analysis(request: IdeaAnalysisChatRequest) -> dict[str, Any]:
    if not llm_settings()["configured"]:
        raise HTTPException(503, "请先在工作台顶部配置全局 LLM")
    session = get_idea_session(request.session_id) if request.session_id else None
    if request.session_id and not session:
        raise HTTPException(404, "想法会话不存在")
    if not session:
...
def get_project_agent_session(project_id: str, session_id: str) -> dict[str, Any]:
    ...
    return {"session": session, "messages": list_agent_messages(session_id), "agent": agent_detail(project_id, llm_ready=bool(llm_settings()["configured"]))}
...
def get_project_agent_work_items(project_id: str, status: str = "open", limit: int = 20) -> dict[str, Any]:
    ...
    if status not in {"all", "open", "running", "blocked", "done", "failed", "archived"}:
        raise HTTPException(400, "不支持的工作项状态")
    items = list_work_items(status=status, project_id=project_id)
    items = [item for item in items if project_id in {part.strip() for part in str(item.get("target_project", "")).split(",") if part.strip()}]
    return {"items": items[: max(1, min(limit, 100))], "project_id": project_id, "agent": agent_detail(project_id, llm_ready=bool(llm_settings()["configured"]))}
...
async def run_project_work_item(project_id: str, item_id: int) -> dict[str, Any]:
    ...
    if project_id in {"knowledge", "doc-factory"} and item.get("source_project") == "inbox":
        return await run_inbox_handoff_work_item(project_id, item)
    session = create_agent_session(project_id, f"交接：{clip(item.get('title', '未命名工作项'), 90)}")
    handoff_message = (
...
def list_agents() -> dict[str, Any]:
    ...
    global_agent = agent_detail("workbench", llm_ready=configured)
    global_agent["id"] = "workbench"
    global_agent["children"] = child_ids
    global_agent["children_detail"] = [agent_detail(project_id, llm_ready=configured) for project_id in child_ids]
    return {
        "llm": llm_settings(),
        "global_agent": global_agent,
        "agents": [agent_detail(project_id, llm_ready=configured) for project_id in project_ids],
    }
...
def knowledge_vector_features(value: str) -> set[str]:
    ...
    features = {f"t:{token}" for token in knowledge_tokens(text)}
...
def obsidian_semantic_results(query: str, limit: int = 20) -> list[dict[str, Any]]:
    query_tokens = knowledge_tokens(query)
    if not query_tokens:
        return obsidian_search("", limit=limit)
    rows = obsidian_index_rows()
    query_vector = knowledge_hash_vector(knowledge_vector_features(query))
    ...
    for row in rows:
        ...
        tokens = knowledge_tokens(text)
        features = knowledge_vector_features(text)
        absolute = str(OBSIDIAN_VAULT_DIR / str(note.get("path") or ""))
        document_vector = semantic_vectors.get(absolute) if semantic_query_vector else None
        documents.append((row, note, text, tokens, features, knowledge_hash_vector(features), document_vector, absolute))
        for token in tokens:
    ...
    for row, note, text, note_tokens, _features, document_vector, semantic_doc_vector, absolute in documents:
        ...
        if semantic_query_vector and semantic_doc_vector:
            vector_score = _cosine_similarity(semantic_query_vector, semantic_doc_vector)
        else:
            vector_score = knowledge_vector_similarity(query_vector, document_vector)
        ...
        path = OBSIDIAN_VAULT_DIR / str(note.get("path") or "")
        locators = []
        try:
...
def save_knowledge_conflict_paragraph_resolution(
    ...
    paragraph_key = str(paragraph.get("paragraph_key") or obsidian_conflict_paragraph_key(conflict_key, paragraph)).strip()
    left = paragraph.get("left") if isinstance(paragraph.get("left"), dict) else {}
    right = paragraph.get("right") if isinstance(paragraph.get("right"), dict) else {}
    timestamp = now_iso()
    connection = db_connection()
...
def knowledge_conflict_draft(conflict: dict[str, Any], resolution: dict[str, Any]) -> dict[str, Any] | None:
    ...
    left = conflict.get("left") if isinstance(conflict.get("left"), dict) else {}
    right = conflict.get("right") if isinstance(conflict.get("right"), dict) else {}
    left_path = OBSIDIAN_VAULT_DIR / str(left.get("path") or "")
    right_path = OBSIDIAN_VAULT_DIR / str(right.get("path") or "")
    try:
...
def knowledge_conflict_paragraph_draft(conflict: dict[str, Any], paragraph: dict[str, Any], resolution: dict[str, Any]) -> dict[str, Any] | None:
    ...
    path = OUTPUTS_DIR / f"{datetime.now().strftime('%Y%m%d-%H%M%S-%f')}-{safe_filename(title, 'knowledge-conflict-paragraph')}.md"
    path.write_text(body, encoding="utf-8")
    return register_artifact_safely(
        project_id="knowledge",
        name=path.name,
        path=str(path),
...
def obsidian_paragraph_conflicts(left: dict[str, Any], right: dict[str, Any], contradiction_pairs: tuple[tuple[str, str], ...], conflict_key: str | None = None) -> list[dict[str, Any]]:
    ...
    left_blocks = _obsidian_paragraph_blocks(left)
    right_blocks = _obsidian_paragraph_blocks(right)
    matches: list[dict[str, Any]] = []
    for left_block in left_blocks:
        left_tokens = knowledge_tokens(left_block["text"])
        if len(left_tokens) < 3:
            continue
        left_text = left_block["text"].lower()
        for right_block in right_blocks:
            right_tokens = knowledge_tokens(right_block["text"])
            ...
            match["paragraph_key"] = obsidian_conflict_paragraph_key(conflict_key or obsidian_conflict_key(left, right), match)
...
def obsidian_conflict_report() -> dict[str, Any]:
    notes = [obsidian_note_row(row) for row in obsidian_index_rows()]
    resolutions = list_knowledge_conflict_resolutions()
    ...
    for index, first in enumerate(notes):
        left = knowledge_tokens(f"{first.get('title')} {first.get('preview')}")
        if len(left) < 3:
            continue
        for second in notes[index + 1:]:
            right = knowledge_tokens(f"{second.get('title')} {second.get('preview')}")
            ...
            if similarity >= 0.62 and first.get("content_hash") != second.get("content_hash"):
                ...
                contradiction = next((pair for pair in contradiction_pairs if (pair[0] in left_text and pair[1] in right_text) or (pair[1] in left_text and pair[0] in right_text)), None)
                key = obsidian_conflict_key(first, second)
                paragraph_conflicts = obsidian_paragraph_conflicts(first, second, contradiction_pairs, key)
                paragraph_resolutions = list_knowledge_conflict_paragraph_resolutions(key)
...
def obsidian_retrieval_evaluation(sample_limit: int = 30, top_k: int = 5) -> dict[str, Any]:
    ...
    for row in rows[: max(1, min(sample_limit, 100))]:
        ...
        if len(knowledge_tokens(title)) < 1:
            skipped_count += 1
            continue
        target = str(note.get("path") or "")
        baseline = obsidian_search(title, limit=top_k)
        hybrid = obsidian_semantic_results(title, limit=top_k)
...
def obsidian_moc_preview() -> dict[str, Any]:
    status = obsidian_moc_suggestions()
...
def apply_obsidian_moc(note_paths: list[str]) -> dict[str, Any]:
    ...
    safe = [path for path in candidates if OBSIDIAN_VAULT_DIR.resolve() in path.parents and path.is_file()]
    moc_paths = [path for path in obsidian_note_paths() if "moc" in path.stem.lower() or "moc" in str(path.parent).lower()]
    if not moc_paths:
...
def resolve_obsidian_conflict(conflict_key: str, request: ObsidianConflictResolutionRequest) -> dict[str, Any]:
    ...
    resolution = save_knowledge_conflict_resolution(conflict, request.action, request.note)
    artifact = register_artifact_safely(project_id="knowledge", name=f"知识冲突处理 · {conflict_key}", kind="knowledge_conflict_resolution", metadata={"conflict_key": conflict_key, "action": request.action, "note": request.note, "left_path": conflict.get("left", {}).get("path", ""), "right_path": conflict.get("right", {}).get("path", "")})
    relations = []
    if artifact:
        for side, label in ((conflict.get("left") or {}, "left"), (conflict.get("right") or {}, "right")):
            if side.get("path"):
                relations.append(create_relation_record(from_type="artifact", from_id=str(artifact.get("id")), to_type="obsidian_note", to_id=str(side.get("path")), relation_type="conflict_resolution_source", metadata={"side": label, "action": request.action, "conflict_key": conflict_key}))
    draft = knowledge_conflict_draft(conflict, resolution)
    if draft and artifact and draft.get("id"):
        relations.append(create_relation_record(from_type="artifact", from_id=str(artifact.get("id")), to_type="artifact", to_id=str(draft.get("id")), relation_type="conflict_to_merge_draft", metadata={"conflict_key": conflict_key}))
...
def resolve_obsidian_conflict_paragraph(conflict_key: str, paragraph_key: str, request: ObsidianConflictParagraphResolutionRequest) -> dict[str, Any]:
    ...
    resolution = save_knowledge_conflict_paragraph_resolution(conflict, paragraph, request.action, request.note)
    artifact = register_artifact_safely(
        project_id="knowledge",
    ...
    draft = knowledge_conflict_paragraph_draft(conflict, paragraph, resolution)
    relations = []
    for side, label in ((paragraph.get("left") or {}, "left"), (paragraph.get("right") or {}, "right")):
        if side.get("path") and artifact:
            relations.append(create_relation_record(from_type="artifact", from_id=str(artifact.get("id")), to_type="obsidian_note", to_id=str(side.get("path")), relation_type="conflict_paragraph_source", metadata={"side": label, "action": request.action, "conflict_key": conflict_key, "paragraph_key": paragraph_key, "line_start": side.get("line_start"), "line_end": side.get("line_end")}))
    if draft and artifact and draft.get("id"):
        relations.append(create_relation_record(from_type="artifact", from_id=str(artifact.get("id")), to_type="artifact", to_id=str(draft.get("id")), relation_type="conflict_paragraph_to_review_draft", metadata={"conflict_key": conflict_key, "paragraph_key": paragraph_key, "action": request.action}))
...
def maintain_obsidian_moc(request: ObsidianMocApplyRequest) -> dict[str, Any]:
    ...
    try:
        result = apply_obsidian_moc(request.note_paths)
    except ValueError as exc:
        raise HTTPException(409, str(exc)) from exc
    artifact = register_artifact_safely(project_id="knowledge", name=Path(result["path"]).name, path=result["path"], kind="obsidian_moc_update", metadata={"backup": result["backup"], "added": result["added"]})
...
def create_knowledge_source_draft(request: KnowledgeSourceDraftRequest) -> dict[str, Any]:
    ...
    note = write_knowledge_note(
        ...
        metadata={
            ...
            "source_content_hashes": {str(item["artifact"].get("id")): hashlib.sha256(read_artifact_source(item["artifact"])[0].encode("utf-8", errors="ignore")).hexdigest() for item in sources},
            "review_required": True,
        },
    ...
    for item in sources:
        if note.get("artifact"):
            create_relation_record(from_type="artifact", from_id=str(item["artifact"].get("id")), to_type="artifact", to_id=str(note["artifact"].get("id")), relation_type="source_to_review_draft", metadata={"review_required": True})
...
def create_knowledge_selection_draft(request: KnowledgeSelectionDraftRequest) -> dict[str, Any]:
    ...
    source_path, _ = artifact_source_path(artifact)
    ...
    if note.get("artifact"):
        create_relation_record(from_type="artifact", from_id=str(request.artifact_id), to_type="artifact", to_id=str(note["artifact"]["id"]), relation_type="selected_source_to_draft", metadata={"locators": [item["locator"] for item in selections]})
...
def market_backtest_input_error(symbol: Any, strategy: Any) -> str:
    if not normalize_market_symbol(symbol):
        return "无法识别股票代码，请填写 6 位 A 股/ETF 代码或 sh/sz 前缀。"
    if not normalize_market_strategy(strategy):
...
def market_history_series(symbol: str) -> list[dict[str, Any]]:
    ...
    for row in list_market_history(limit=200):
        checked_at = row.get("checked_at")
        quote = next((item for item in row.get("quotes", []) if normalize_market_symbol(item.get("symbol", "")) == normalized), None)
        quality = market_quote_quality(quote or {})
        if quote and quality.get("valid") and quality.get("price") is not None:
            by_timestamp[market_timestamp_key(checked_at)] = {"checked_at": checked_at, "price": float(quality["price"]), "volume": quality.get("volume")}
    return sorted(
        by_timestamp.values(),
...
def market_backtest_samples(symbol: str, snapshot: dict[str, Any] | None = None) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    entries = [(row, False) for row in list_market_history(limit=200)]
    ...
    for row, is_current in entries:
        ...
        if not quote or not quality.get("valid"):
            ...
            continue
        timestamp_key = market_timestamp_key(checked_at)
...
def _select_walk_forward_window(
    ...
    candidates = _walk_forward_candidate_windows(window, len(train_points))
    evaluations = []
    for candidate in candidates:
        result = _simulate_market_backtest(train_points, strategy, candidate, fee_bps, slippage_bps)
...
def market_walk_forward(
    ...
    if not _walk_forward_candidate_windows(window, train_size):
    ...
    while start + train_size + test_size <= len(points) and len(folds) < max_folds:
        ...
        selection = _select_walk_forward_window(train_points, strategy, window, fee_bps, slippage_bps)
        selected_window = selection.get("selected_window")
        if selected_window is None:
            break
        block = train_points + test_points
        oos = _simulate_market_backtest(block, strategy, selected_window, fee_bps, slippage_bps, evaluation_start=len(train_points))
        gross = _simulate_market_backtest(block, strategy, selected_window, 0, 0, evaluation_start=len(train_points))
        folds.append({
            "fold": len(folds) + 1,
    ...
    sharpe, sortino = _fold_sample_ratios(oos_returns)
    ...
    return {
        **base,
        ...
        "out_of_sample_return_pct": _compound_percent_returns(oos_returns),
        "out_of_sample_benchmark_return_pct": _compound_percent_returns(benchmark_returns),
        "out_of_sample_active_return_pct": round(_compound_percent_returns(oos_returns) - _compound_percent_returns(benchmark_returns), 4),
        "out_of_sample_max_drawdown_pct": _fold_max_drawdown(oos_returns),
...
def market_backtest(
    ...
    simulation = _simulate_market_backtest(points, strategy, window, fee_bps, slippage_bps)
    gross = _simulate_market_backtest(points, strategy, window, 0, 0) if fee_bps or slippage_bps else None
    return {
        **base,
        "status": "ok",
...
def create_market_research(request: MarketResearchRequest) -> dict[str, Any]:
    ...
    artifact = register_artifact_safely(project_id="market", name=f"行情研究上下文 · {symbol or '全市场'}", path=str(MARKET_SNAPSHOT_FILE), kind="market_research_context", metadata={"symbol": symbol, "checked_at": snapshot.get("checked_at"), "question": request.question, "data_quality": data_quality})
    quality_note = f"数据质量：{data_quality['freshness_label']} · 来源 {data_quality['source']} · 历史快照 {data_quality['history_count']} 次"
    item = create_work_item_record(title=f"行情研究：{clip(request.question, 120)}", description=f"标的：{symbol or '未指定'}\n问题：{request.question}\n事件日期：{request.event_date or '未指定'}\n数据时间：{snapshot.get('checked_at') or '未知'}\n{quality_note}\n要求：请结合历史快照与公开来源完成事件驱动研究，不自动交易。", kind="market_event_research", source_project="market", target_project="knowledge", metadata={"symbol": symbol, "question": request.question, "event_date": request.event_date, "artifact_id": artifact.get("id") if artifact else None, "checked_at": snapshot.get("checked_at"), "data_quality": data_quality})
    relation = create_relation_record(from_type="artifact", from_id=str(artifact.get("id")), to_type="work_item", to_id=str(item.get("id")), relation_type="market_context_to_research", metadata={"symbol": symbol, "data_quality": data_quality}) if artifact else None
...
def create_market_report() -> dict[str, Any]:
    ...
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    artifact = register_artifact_safely(project_id="market", name=path.name, path=str(path), kind="market_report", metadata={"report_type": "daily", "checked_at": snapshot.get("checked_at"), "history_count": len(history)})
...
def run_market_backtest(request: MarketBacktestRequest) -> dict[str, Any]:
    ...
    snapshot = load_market_snapshot()
    record_market_snapshot(snapshot)
    result = market_backtest(symbol, strategy, request.window, request.fee_bps, request.slippage_bps, snapshot=snapshot)
    artifact = register_artifact_safely(project_id="market", name=f"回测-{symbol}-{strategy}-{datetime.now().strftime('%Y%m%d%H%M%S')}.json", path=str(MARKET_SNAPSHOT_FILE), kind="market_backtest", metadata=result)
...
def compare_market_strategies(request: MarketStrategyCompareRequest) -> dict[str, Any]:
    ...
    invalid = [item for item in strategies if not normalize_market_strategy(item)]
    if invalid:
    ...
    record_market_snapshot(snapshot)
    comparison = [
        market_backtest(symbol, normalize_market_strategy(strategy), request.window, request.fee_bps, request.slippage_bps, snapshot=snapshot)
        for strategy in strategies
    ]
    artifact = register_artifact_safely(
        project_id="market",
        name=f"策略对比-{normalize_market_symbol(request.symbol)}-{datetime.now().strftime('%Y%m%d%H%M%S')}.json",
        path=str(MARKET_SNAPSHOT_FILE),
...
def market_backtest_sensitivity(request: MarketSensitivityRequest) -> dict[str, Any]:
    ...
    for label, fee_bps, slippage_bps in scenarios:
        result = market_backtest(symbol, strategy, request.window, fee_bps, slippage_bps, snapshot=snapshot)
...
def idea_followups_payload(days: int = 3) -> dict[str, Any]:
    ...
    tasks = []
    for row in rows:
        task = idea_validation_task_row(row)
        ...
        task["work_item"] = get_work_item_record(int(task.get("work_item_id") or 0)) if task.get("work_item_id") else None
        tasks.append(task)
    return {
...
def idea_session_artifacts(session_id: str, kinds: set[str] | None = None) -> list[dict[str, Any]]:
    result = []
    for artifact in list_artifacts("idea-analysis"):
        metadata = artifact.get("metadata") if isinstance(artifact.get("metadata"), dict) else {}
...
def add_idea_evidence(session_id: str, request: IdeaEvidenceRequest) -> dict[str, Any]:
    ...
    if not session:
        raise HTTPException(404, "想法会话不存在")
    artifact = register_artifact_safely(project_id="idea-analysis", name=f"{safe_filename(request.title)}-{datetime.now().strftime('%Y%m%d%H%M%S')}.md", path="", kind="idea_evidence", metadata={"session_id": session_id, "evidence_kind": request.kind, "source": request.source, "status": request.status, "title": request.title})
    if artifact:
        ...
        try:
            ...
            connection.close()
        artifact = get_artifact_record(int(artifact["id"])) or artifact
        create_relation_record(from_type="idea_session", from_id=session_id, to_type="artifact", to_id=str(artifact["id"]), relation_type="session_to_evidence", metadata={"kind": request.kind, "status": request.status})
...
def add_idea_interview(session_id: str, request: IdeaInterviewRequest) -> dict[str, Any]:
    ...
    artifact = register_artifact_safely(
        project_id="idea-analysis",
        name=f"访谈-{safe_filename(request.participant)}-{datetime.now().strftime('%Y%m%d%H%M%S')}.md",
        path="",
        kind="idea_interview",
    ...
    if artifact:
        ...
        try:
            ...
            connection.close()
        artifact = get_artifact_record(int(artifact["id"])) or artifact
        create_relation_record(from_type="idea_session", from_id=session_id, to_type="artifact", to_id=str(artifact["id"]), relation_type="session_to_interview", metadata={"status": request.status, "participant": request.participant})
...
def add_idea_metric(session_id: str, request: IdeaMetricRequest) -> dict[str, Any]:
    if not get_idea_session(session_id):
        raise HTTPException(404, "想法会话不存在")
    artifact = register_artifact_safely(project_id="idea-analysis", name=f"指标-{safe_filename(request.name)}-{datetime.now().strftime('%Y%m%d%H%M%S')}.json", path="", kind="idea_metric", metadata={"session_id": session_id, "name": request.name, "value": request.value, "target": request.target, "status": request.status, "note": request.note})
    if artifact:
        ...
        try:
            ...
            connection.close()
        artifact = get_artifact_record(int(artifact["id"])) or artifact
        create_relation_record(from_type="idea_session", from_id=session_id, to_type="artifact", to_id=str(artifact["id"]), relation_type="session_to_metric", metadata={"status": request.status})
    return {"ok": True, "artifact": artifact}
...
def compare_idea_decision(session_id: str, request: IdeaDecisionCompareRequest) -> dict[str, Any]:
    ...
    if request.confirmed:
        decision = create_idea_decision_record(session_id=session_id, verdict=request.verdict.strip() or automatic, rationale=request.rationale.strip() or f"基于 {supported} 条支持证据、{contradicted} 条反证和 {unknown} 条未决证据的版本化比较。", continue_if="支持证据持续增加且关键指标达到目标。", stop_if="反证超过支持证据，或关键指标连续未达标。", evidence={"supported": supported, "contradicted": contradicted, "unknown": unknown, "automatic_suggestion": automatic})
        session = get_idea_session(session_id) or {"title": "想法验证"}
        ...
        if knowledge_note.get("artifact"):
            create_relation_record(from_type="idea_decision", from_id=str(decision.get("id")), to_type="artifact", to_id=str(knowledge_note["artifact"].get("id")), relation_type="decision_to_knowledge", metadata={"session_id": session_id})
    return {
        "comparison": {
            "supported": supported,
...
def get_idea_evidence_pack(session_id: str) -> dict[str, Any]:
    ...
    if not session:
        raise HTTPException(404, "想法会话不存在")
    evidence = idea_session_artifacts(session_id, {"idea_evidence", "idea_interview", "idea_metric", "idea_decision_note"})
    decision = list_idea_decisions(session_id)
...
def create_market_valuation(request: MarketValuationRequest) -> dict[str, Any]:
    ...
    result = {"symbol": symbol, "available": available, "factors": factors, "missing": [key for key in ("pe", "pb", "roe") if key not in numeric], "note": request.note.strip(), "data_as_of": load_market_snapshot().get("checked_at", ""), "policy": "估值因子只接受明确来源的输入；缺数据时不推断估值，也不构成投资建议。"}
    artifact = register_artifact_safely(project_id="market", name=f"估值因子 · {symbol}", kind="market_valuation", path=str(MARKET_SNAPSHOT_FILE), metadata=result)
...
def conclude_market_research(item_id: int, request: MarketResearchConclusionRequest) -> dict[str, Any]:
    ...
    updated = update_work_item_record(item_id, {"status": "done", "completed_at": now_iso(), "metadata_json": json.dumps(updated_metadata, ensure_ascii=False), "result_json": json.dumps({"knowledge_artifact_id": conclusion_artifact_id, "conclusion": request.conclusion.strip()}, ensure_ascii=False), "last_error": ""})
...
async def _tencent_convertible_quotes(codes: list[str]) -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        for line in text.splitlines():
            ...
            price = parse_market_number(fields[3])
            change_pct = parse_market_number(fields[32] if len(fields) > 32 else None)
            # 转股溢价率在部分代码段不返回，尽力而为
            premium = parse_market_number(fields[47] if len(fields) > 47 else None)
            result[code] = {
                "price": price,
