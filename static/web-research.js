...
function markdownLight(text) {
  // 统一走 markdown.js：这里原来这份没有表格支持，而 Agent 的回答最爱用表格。
  // 两份渲染器并存迟早会出现「同一段内容在两个面板里长得不一样」。
  if (window.WorkbenchMarkdown) return window.WorkbenchMarkdown.render(text);
  return markdownLightFallback(text);
}

function markdownLightFallback(text) {
...
async function copilotAsk(message) { const context = activeContext(); if (!message.trim() || !context) return; if (!researchAnswerReady()) { appendMessage("user", message, { kind: "chat" }); const input = $("#chat-input"); if (input) input.value = ""; appendMessage("assistant", "当前网页已经可以操作，但内容问答还在准备。你可以先让我点击、输入或滚动；页面读完后会自动开放总结和追问。", { kind: "notice" }); return; } const contextId = context.id; const runId = context.runId; appendMessage("user", message, { kind: "chat", runId }); const input = $("#chat-input"); if (input) input.value = ""; const thinking = appendThinking(); const submit = $("#chat-submit"); setBusy(submit, true, "…"); const host = $("#copilot-messages"); const item = document.createElement("div"); item.className = "chat-message assistant"; const label = document.createElement("span"); label.className = "chat-label"; label.textContent = "AI"; const bodyEl = document.createElement("div"); bodyEl.className = "chat-body"; item.append(label, bodyEl); host?.append(item); thinking.remove(); let lastText = ""; try { let liveContext = ""; if (nativeBrowserAvailable() && viewMode === "iframe") { const live = await nativeBrowser.snapshot(context.id); if (live?.ok && live.snapshot) liveContext = nativeSnapshotContext(live.snapshot); } let answer = ""; if (window.WorkbenchUX?.fetchStream) { answer = await window.WorkbenchUX.fetchStream("/api/chat", { run_id: runId, message, live_context: liveContext, stream: true }, { onDelta: (t) => { if (t) { lastText += t; if (contextId === state.activeId && activeContext()?.runId === runId) { bodyEl.innerHTML = markdownLight(lastText); if (host) host.scrollTop = host.scrollHeight; } } }, onReset: () => { lastText = ""; if (contextId === state.activeId) bodyEl.textContent = "备用 Provider 正在重新回答…"; }, onError: (msg, payload) => { if (!payload?.recoverable && !lastText && contextId === state.activeId) bodyEl.textContent = `追问失败：${msg}`; } }); } else { const body = await window.requestJson("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ run_id: runId, message, live_context: liveContext }) }); answer = body.answer || "没有返回内容"; if (contextId === state.activeId && activeContext()?.runId === runId) bodyEl.innerHTML = markdownLight(answer); } if (answer && context.messages) { context.messages = [...(context.messages || []), { role: "assistant", content: answer, kind: "chat", runId, at: new Date().toISOString() }].slice(-40); saveContexts(); } } catch (error) { if (contextId === state.activeId && !lastText) { bodyEl.textContent = `追问失败：${error.message}`; } } finally { setBusy(submit, false); } }
