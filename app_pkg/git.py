...
def get_github_tools() -> dict[str, Any]:
    ...
    tools = [
        {"id": "markitdown", "name": "Microsoft MarkItDown", "url": "https://github.com/microsoft/markitdown", "scenario": "文档、网页和演示材料转 Markdown", "cost": "免费·可选依赖", "fit": "已接入文档工厂；未安装时回退内置解析器", "trial": "在文档工厂上传一份 PPTX 或复杂表格，检查 Markdown 结构和来源保留", "state": "integrated", "installed": bool(markitdown.get("available")), "data_boundary": "文件只在 NEXUS 进程内转换，不自动上传"},
        ...
        {"id": "lazygit", "name": "lazygit", "url": "https://github.com/jesseduffield/lazygit", "scenario": "终端 Git 审查与提交", "cost": "免费·本地", "fit": "NEXUS 已有只读 Git 项目中心；lazygit 作为本机审查补充", "trial": "安装后扫描一个仓库并完成一次分支审查；NEXUS 不自动执行提交或 push", "state": "candidate", "installed": bool(shutil.which("lazygit")), "data_boundary": "本地 Git 元数据；不要把密钥或完整 diff 放入通知"},
